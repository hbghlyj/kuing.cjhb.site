...
<tr>
			...
			<!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}-->
				<!--{if $_G['setting']['authoronleft']}-->
					<div class="pi">
						...
						<!--{if $post['first']}--><div class="authi"><em>{lang show}: <!--{echo dnumber($_G[forum_thread][views])}--></em><span class="pipe">|</span><em>{lang reply}: <!--{echo dnumber($_G[forum_thread][allreplies])}--></em></div><!--{/if}-->
					</div>
				<!--{/if}-->
			...
			<!--{if !IS_ROBOT}-->
				<!--{if !$postcount && !$_G['forum_thread']['archiveid'] && $post['first'] && $_G['style']['viewthread_pl']}-->
					<div id="fj" class="y">
						...
						<a href="javascript:;" id="fj_btn" class="z" title="{lang thread_redirect_postno_tips}"><img src="{IMGDIR}/fj_btn.svg" alt=""></a>
					</div>
				<!--{/if}-->
				<!--{if $post['warned']}-->
	...
	<tr><td class="plc plm">
		...
		<!--{if !IS_ROBOT && $post['first'] && !$_G['forum_thread']['archiveid']}-->
			...
			<!--{if $post['invisible'] == 0 && $_G['style']['viewthread_pl']}-->
		...
		<!--{if !empty($post['relateitem'])}-->
			<div class="mtw mbw">
				...
				<ul class="xl cl relatedthreadlist">
		...
		<div class="po hin">
			...
			<div class="pob cl">
				<em>
					...
					<!--{if !empty($_G['setting']['recommendthread']['status']) && !$_G['forum_thread']['special'] && !$hiddenreplies && $_G['setting']['repliesrank'] && !$post['first'] && !($post['isWater'] && $_G['setting']['filterednovote'])}-->
					<a class="replyadd{if $post[postreview][supported]} active{/if}" {if $_G['uid']}href="forum.php?mod=misc&action=postreview&do=support&tid=$_G[tid]&pid=$post[pid]&hash={FORMHASH}" onclick="ajaxmenu(this, 3000, 0, 0, '43', '');return false;"{else} onclick="showWindow('login','member.php?mod=logging&action=login')"{/if}{if $post[postreview][support]} title="$post[postreview][support] {lang activity_member_unit}&#13;$post[postreview]['support_member']"{/if}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg><span id="review_support_$post[pid]" class="xi1"{if !$post[postreview][support]} style="display:none"{/if}>$post[postreview][support]</span></a>
					<a class="replysubtract{if $post[postreview][opposed]} active{/if}" {if $_G['uid']}href="forum.php?mod=misc&action=postreview&do=against&tid=$_G[tid]&pid=$post[pid]&hash={FORMHASH}" onclick="ajaxmenu(this, 3000, 0, 0, '43', '');return false;"{else} onclick="showWindow('login','member.php?mod=logging&action=login')"{/if}{if $post[postreview][against]} title="$post[postreview][against] {lang activity_member_unit}&#13;$post[postreview]['against_member']"{/if}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h3a2 2 0 0 1 2 2v7a2 2 0 0 1-2 2h-3"></path></svg><span id="review_against_$post[pid]" class="xi1"{if !$post[postreview][against]} style="display:none"{/if}>$post[postreview][against]</span></a>
					<!--{/if}-->
				...
				<p>
					<!--{if $post['invisible'] == 0}-->
						...
						<!--{if $_G['uid'] && $post['authorid'] == $_G['uid'] && $_G['setting']['editperdel']}-->
							<a class="removep" href="forum.php?mod=misc&action=postdelete&tid=$post[tid]&pid=$post[pid]&extra=$_GET[extra]&page=$page&infloat=yes&handlekey=postdelete$post[pid]" onclick="showWindow('postdelete$post[pid]', this.href, 'get', 0);return false;">{lang delete}</a>
						<!--{else}-->
							<a class="report-action" href="javascript:;" onclick="showWindow('miscreport$post[pid]', 'misc.php?mod=report&rtype=post&rid=$post[pid]&tid=$_G[tid]&fid=$_G[fid]', 'get', -1);return false;"><svg class="report-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"></path><path d="M12 8v4"></path><path d="M12 16h.01"></path></svg> {lang report}</a>
						<!--{/if}-->
					<!--{/if}-->
