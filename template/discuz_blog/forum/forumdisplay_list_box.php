				...
				<!--{loop $_G['forum_threadlist'] $key $thread}-->
						...
						<li{if CURMODULE == 'forumdisplay'} id="$thread['id']"{/if} class="kmlist $thread['folder']"{if $_G['hiddenexists'] && $thread['hidden']} style='display:none'{/if}>
							...
							<a href="forum.php?mod=viewthread&tid=$thread['tid']&{if $_GET['archiveid']}archiveid={$_GET['archiveid']}&{/if}extra=$extra"$thread['highlight']{if $thread['isgroup'] == 1 || $thread['forumstick']} target="_blank"{else} onclick="atarget(this)"{/if} title="{if $thread['displayorder'] == 1}{lang thread_type1}{/if}
									...
									{if $thread['special'] == 1}{lang thread_poll}{lang sp}{lang forum_threads}{/if}
									{if $thread['special'] == 2}{lang thread_trade}{lang sp}{lang forum_threads}{/if}
									{if $thread['special'] == 3}{lang thread_reward}{lang sp}{lang forum_threads}{/if}
									{if $thread['special'] == 4}{lang thread_activity}{lang sp}{lang forum_threads}{/if}
									{if $thread['special'] == 5}{lang thread_debate}{lang sp}{lang forum_threads}{/if}
									{if $thread['folder'] == "new"}{lang have_newreplies}{lang sp}{lang forum_threads}{/if}">
							...
							<div class="kmmeta">
								...
								<!--{/if}-->
								<span class="kmtime{if $thread['istoday'] && CURMODULE == 'forumdisplay'} xi1{/if}">{lang started} <time data-local-timestamp="$thread['dbdateline']">$thread['dateline']</time></span>
								</cite>
