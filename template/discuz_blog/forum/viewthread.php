...
<div class="wp sg-viewthread-layout">
	<main class="sg-viewthread-main">
<div id="ct" class="cl">
	<!--{if !$_G['style']['blog_mod']}-->
	<div id="pgt" class="pgs mbm cl {if $modmenu['thread']}pbm bbs{/if}">
		...
		<!--{if $_G['forum']['threadsorts'] && $_G['forum']['threadsorts']['templatelist']}-->
			...
			<!--{/loop}-->
		<!--{else}-->
			<!--{if !$_G['forum_thread']['is_archived']}--><a id="newspecial" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && empty($_G['forum']['threadsorts']['required'])} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} href="javascript:;" title="{lang post_newthread}" class="pgsbtn showmenu">{lang post_newthread}</a><!--{/if}-->
		<!--{/if}-->
...
<div id="postlist" class="pl bm">
	<!--{if $_G['style']['blog_mod']}-->
	<!--{eval}-->
		$sg_first_post = !empty($_G['forum_firstpid']) && !empty($postlist[$_G['forum_firstpid']]) ? $postlist[$_G['forum_firstpid']] : array();
		$sg_thread_cover = '';
		if (!empty($_G['forum_thread']['cover'])) {
			$sg_thread_cover = getthreadcover($_G['tid'], $_G['forum_thread']['cover']);
		}

		if (!$sg_thread_cover && !empty($sg_first_post['attachments'])) {
			foreach ($sg_first_post['attachments'] as $sg_attachment) {
				if (!empty($sg_attachment['attachimg']) && !empty($sg_attachment['attachment'])) {
					if ($sg_attachment['ext']=='mp4') {
						$sg_thread_cover = $sg_attachment['url'].$sg_attachment['attachment'].'.thumb.jpg';
					}else{
						$sg_thread_cover = $sg_attachment['url'].$sg_attachment['attachment'];
					}
		...
		if (!empty($_G['forum_thread']['digest'])) {
			...
			$sg_thread_label_url = 'forum.php?mod=forumdisplay&fid='.$_G['fid'].'&filter=typeid&typeid='.$_G['forum_thread']['typeid'];
		} elseif (!empty($_G['forum_thread']['sortid']) && !empty($_G['forum']['threadsorts']['types'][$_G['forum_thread']['sortid']])) {
			$sg_thread_label = strip_tags($_G['forum']['threadsorts']['types'][$_G['forum_thread']['sortid']]);
			$sg_thread_label_url = 'forum.php?mod=forumdisplay&fid='.$_G['fid'].'&filter=sortid&sortid='.$_G['forum_thread']['sortid'];
		} else {
			$sg_thread_label = strip_tags($_G['forum']['name']);
	...
	<!--{loop $postlist $post}-->
        ...
        <div id="post_$post[pid]" class="{if $post['first']}sg-first-post{else}sg-reply-post{/if}" {if $_G['blockedpids'] && $post['inblacklist']}style="display:none;"{/if}>
            <!--{subtemplate forum/viewthread_node}-->
        </div>
		<!--{else}-->
			<!--{if $post['first']}-->
				<div id="post_$post[pid]" class="sg-first-post" {if $_G['blockedpids'] && $post['inblacklist']}style="display:none;"{/if}>
					<!--{subtemplate forum/viewthread_node}-->
				</div>
			<!--{/if}-->
		<!--{/if}-->
...
<div class="pgs mtm mbm cl">
	...
	<!--{if !$_G['forum_thread']['is_archived']}-->
		<a id="newspecialtmp" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && empty($_G['forum']['threadsorts']['required'])} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} href="javascript:;" title="{lang post_newthread}" class="pgsbtn showmenu">{lang post_newthread}</a>
	<!--{/if}-->
	<!--{if $allowpostreply && !$_G['forum_thread']['archiveid']}-->
...
<!--{if $fastpost && $_G['style']['viewthread_pl']}-->
