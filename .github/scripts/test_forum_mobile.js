...
(async () => {
    ...
    try {
        ...
        assert.strictEqual(await tagInput.count(), 1, 'Assertion Error: Mobile tag input was not initially visible.');
        await extraTagBtn.click();
        assert.strictEqual(await tagInput.count(), 0, 'Assertion Error: Mobile tag control did not close the tag input.');
        await extraTagBtn.click();
        ...
        try {
            ...
            if (token) {
                ...
                const sha2 = process.env.GITHUB_SHA || suffix;
                await fetch(`https://api.github.com/repos/hbghlyj/kuing.cjhb.site/commits/${sha2}/comments`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json', 'Accept': 'application/vnd.github.v3+json' }, body: JSON.stringify({ body: '**FAIL ' + suffix + ' mobile**\n\n```\n' + gistBody.slice(0,50000) + '\n```' }) }).then(r=>r.json()).then(j=> console.log('COMMENT_CREATED ' + j.html_url)).catch(e=> console.log('comment error', e.message));
            }
        } catch {}
    } finally {
        await browser.close();
