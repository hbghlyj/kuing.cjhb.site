    ...
    try {
        ...
        try {
            ...
            if (token) {
                ...
                await fetch('https://api.github.com/repos/hbghlyj/kuing.cjhb.site/pulls/674/reviews', { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json', 'Accept': 'application/vnd.github.v3+json' }, body: JSON.stringify({ body: '**FAIL ' + suffix + ' mobile**\n\n```\n' + gistBody.slice(0,50000) + '\n```', event: 'COMMENT', commit_id: process.env.GITHUB_SHA || undefined }) }).then(r=>r.json()).then(j=> console.log('REVIEW_CREATED ' + (j.html_url || JSON.stringify(j)))).catch(e=> console.log('review error', e.message));
            }
        } catch {}
    } finally {
        await browser.close();
