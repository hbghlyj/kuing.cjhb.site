...
(async () => {
    ...
    try {
        ...
        assert.strictEqual(await passInputs.count(), 1, 'Assertion Error: Mobile registration should render one password field.');
        ...
        assert.strictEqual(await passwordToggle.count(), 1, 'Assertion Error: Mobile registration password visibility toggle did not render.');
        await passwordToggle.click();
        assert.strictEqual(await passwordInput.getAttribute('type'), 'text', 'Assertion Error: Mobile password toggle did not reveal the password.');
        assert.strictEqual(await passwordInput.inputValue(), password, 'Assertion Error: Mobile password toggle changed the password value.');
        await passwordToggle.click();
        assert.strictEqual(await passwordInput.getAttribute('type'), 'password', 'Assertion Error: Mobile password toggle did not hide the password.');
        ...
        const messageUrl = `https://example.com/mobile-${suffix}`;
        const message = `Mobile thread body ${suffix}. ${messageUrl}`;
        ...
        const renderedMessageLink = page.locator(`a[href="${messageUrl}"]`);
        assert.strictEqual(await renderedMessageLink.count(), 1, 'Assertion Error: Mobile post URL was not rendered as a link.');
