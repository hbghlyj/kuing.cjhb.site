...
$expect(helper_seccheck::rule_register() === [false, true], 'first registration from IP skips image CAPTCHA but keeps secqaa');
C::t('common_regip')->insert(['ip' => $adaptiveCaptchaTestIp, 'count' => 1, 'dateline' => TIMESTAMP - 86401]);
$expect(helper_seccheck::rule_register() === [false, true], 'registration older than 24 hours skips image CAPTCHA but keeps secqaa');
DB::delete('common_regip', DB::field('ip', $adaptiveCaptchaTestIp));
