    ...
    public function create()
    {
        ...
        if ($form->isSuccess()) {
            ...
            if ($this->pageModel->create($values['topic'], $values['filename'])) {
                $form->msg->success(T::trans('Page %topic%/%filename% created successfully!', [
                    '%topic%' => $values['topic'],
                    '%filename%' => $values['filename']
                ]), BASE_URL . '/page/' . $values['topic'] . '/' . $values['filename']);
            } else {
                $form->msg->error(T::trans('There is a file with the same name!'), BASE_URL . '/page/' . $values['topic'] . '/' . $values['filename']);
            }
        }
        return [
            'form' => (string) $form,
