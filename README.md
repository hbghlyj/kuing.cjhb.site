...
### 编辑器模式配置 (Plain Mode vs. JSON Editor Mode)

本分支默认运行于 **Plain Mode（普通/BBCode 编辑器模式）**：

- **Plain Mode**：`common_setting` 中 `defaulteditormode = 0` 且 `editormodetype = 0`。前台发帖/编辑默认使用 Discuz! BBCode 编辑器。在此模式下，后端 `jsoneditorupload.php` 接口按设计要求拒绝未经授权的上传并返回 `{"success": 0}`。
- **JSON Editor Mode**：`editormodetype = 1` 且 `defaulteditormode = 2`。启用后前台使用基于 JSON 块的富文本编辑器，并开放 `misc.php?mod=upload&operation=jsoneditorupload` 附件上传功能。
