...
### JSON 编辑器配置 (editormodetype)

DiscuzX 3.5 引入了基于 JSON 块的富文本编辑器（JSON Editor Mode）：

- `common_setting` 中的 `editormodetype = 1` 表示全局启用 JSON 编辑器模式（对应 `defaulteditormode = 2`）。
- 开启后，前台发帖/编辑将默认使用 JSON 块编辑器，并同时开启对应的后端附件上传接口 `misc.php?mod=upload&operation=jsoneditorupload`。
- 如果 `editormodetype` 为 `0` 且版块未单独配置 JSON 编辑器，后端 `jsoneditorupload.php` 将拒绝文件上传并返回 `{"success": 0}`。
- `tools/import_all_data.php` 种库脚本已在数据库初始化时默认写入 `editormodetype = 1`，保证端到端测试与 JSON 编辑器上传功能正常运行。

### 自动化测试套件与 CI 归档

本分支包含了完整的 Playwright 端到端功能测试套件（`tools/test_*.js`）及自动化 CI 工作流：

- **原生 UI 控件交互**：测试套件（包含注册、发帖、回复、编辑、个人中心、消息、通知、标签、管理员面板及移动端）统一采用 Playwright 原生 UI 交互（`fill()`、`check()`、`click()`）与浏览器内 DOM `fetch()` 执行，不依赖 shadowed/bypass 绕过方法。
- **失败现场 HTML 页面源码自动捕获**：当测试遇到断言失败或页面异常时，测试套件会自动将当前 DOM 页面源码保存为 `*.html`（如 `browser_page_source.html`、`mobile_page_source.html` 等）并截取全屏截图，且已配置由 GitHub Actions artifact 自动归档上传，便于快速定位 CI 运行异常。
