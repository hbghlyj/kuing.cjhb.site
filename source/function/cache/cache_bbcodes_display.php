...
function build_cache_bbcodes_display() {
	$data = [];
	$i = 0;
	foreach(table_forum_bbcode::t()->fetch_all_by_available_icon(2, false) as $bbcode) {
		...
		$tag = $bbcode['tag'];
		$bbcode['i'] = $i;
		$bbcode['icon'] = !empty($bbcode['icon']) ? (preg_match('/^https?:\/\//is', $bbcode['icon']) ? $bbcode['icon'] : STATICURL.'image/common/'.$bbcode['icon']) : '';
