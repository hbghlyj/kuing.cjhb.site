...
function getuserprofile($field) {
	...
	static $tablefields = [
		...
		'field_forum' => ['publishfeed', 'customshow', 'customstatus', 'medals', 'groupterms', 'authstr', 'groups', 'attentiongroup'],
		...
		'profile' => ['realname', 'gender', 'birthyear', 'birthmonth', 'birthday', 'constellation', 'zodiac', 'telephone', 'mobile', 'idcardtype', 'idcard', 'address', 'zipcode', 'nationality', 'birthcountry', 'birthprovince', 'birthcity', 'residecountry', 'resideprovince', 'residecity', 'residedist', 'residecommunity', 'residesuite', 'graduateschool', 'company', 'education', 'occupation', 'position', 'revenue', 'affectivestatus', 'lookingfor', 'bloodtype', 'height', 'weight', 'alipay', 'icq', 'qq', 'yahoo', 'wechat', 'taobao', 'site', 'bio', 'interest', 'field1', 'field2', 'field3', 'field4', 'field5', 'field6', 'field7', 'field8', 'fields'],
...
function checkrobot($client_ip) {
	...
	// Check if client IP is in Internet Archive CIDR ranges
	static $internet_archive_cidrs = ['207.241.224.0/20','207.241.224.0/24','207.241.234.0/24','207.241.237.0/24','208.70.24.0/21'];
	$region_check = _check_ip_in_cidrs($client_ip_long, $internet_archive_cidrs, 'Internet Archive');
	if ($region_check) {
		return $region_check;
	}
	...
	if(strlen($_SERVER['HTTP_ACCEPT']) < 10 || (stripos($_SERVER['HTTP_ACCEPT'], 'text/html') === false && stripos($_SERVER['HTTP_ACCEPT'], '*/*') === false)) {
		...
		if($_SERVER['REQUEST_METHOD'] != 'POST' && empty($_GET['inajax']) && !issecFetchModeCorrectlySet()) {
			return 'UnusualsecFetchModeHeader';
		}
		return false;
	}
}
...
function lang($file, $langvar = null, $vars = [], $default = null) {
	...
	if($path != 'plugin') {
		...
		if(!isset($_G['cache']['pluginlanguage_script'][$file]) && preg_match('/^[a-z]+[a-z0-9_]*$/i', $file)) {
			if(currentlang() == 'EN' && file_exists($loadfile = DISCUZ_PLUGIN($file).'/i18n/SC/lang_plugin.php')) {
				@include $loadfile;
...
function dgmdate($timestamp, $format = 'dt', $timeoffset = 9999, $uformat = '') {
	...
	$format = empty($format) || $format == 'dt' || (IS_ROBOT && empty($_G['inajax'])) ? $dtformat : ($format == 'd' ? $dformat : ($format == 't' ? $tformat : $format));
	if($format == 'u') {
