...
function checkrobot($client_ip) {
	...
	if(strpos($_SERVER['HTTP_USER_AGENT'], 'https://') || strpos($_SERVER['HTTP_USER_AGENT'], 'http://')) {
		define('IS_ROBOT',$_SERVER['HTTP_USER_AGENT']."\t");
		return;
	}
	// Check if client IP is in Alibaba Cloud Singapore/US CIDR ranges
	function _check_ip_in_cidrs($client_ip_long, $cidrs) {
    ...
    if (_check_ip_in_cidrs($client_ip_long, $alibaba_cloud_singapore_cidrs)) {
        define('IS_ROBOT', "Alibaba Cloud Singapore\t");
		return;
    }
    ...
    if (_check_ip_in_cidrs($client_ip_long, $alibaba_cn_net_cidrs)) {
        define('IS_ROBOT', "ALIBABA-CN-NET\t");
		return;
    }
	...
	if (_check_ip_in_cidrs($client_ip_long, $internet_archive_cidrs)) {
...
function lang($file, $langvar = null, $vars = array(), $default = null) {
	...
	if($path != 'plugin') {
		...
		if(defined('IN_MOBILE') && !defined('TPL_DEFAULT')) {
			include DISCUZ_ROOT.'./source/language/'.DISCUZ_LANG.'touch/lang_template.php';
...
function output() {
	...
	if(defined('CACHE_FILE') && CACHE_FILE && !defined('CACHE_FORBIDDEN') && !defined('IN_MOBILE') && !IS_ROBOT && !checkmobile()) {
		if(1/*diskfreespace(DISCUZ_ROOT.'./'.$_G['setting']['cachethreaddir']) > 1000000*/) {
