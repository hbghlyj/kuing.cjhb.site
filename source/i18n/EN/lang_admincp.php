...
(
	...
	'setting_styles_global_creditnotice_comment' => 'Pop up a message to notify the user when their actions change credits',
	...
	'setting_styles_forumdisplay_fastsmilies' => '显示快捷 TeX 代码',
	'setting_styles_forumdisplay_fastsmilies_comment' => '在快速发帖右侧显示快捷 TeX 代码面板',
...
',
	...
	'setting_functions_threadexp_repliesrank' => 'Enable post voting',
	'setting_functions_threadexp_repliesrank_comment' => 'Allow users to support or oppose both opening posts and replies.',
	'setting_functions_threadexp_postreviewdaycount' => 'Post votes per 24 hours',
	'setting_functions_threadexp_postreviewdaycount_comment' => 'Maximum number of posts a user may vote on in 24 hours. Use 0 or leave blank for no limit.',
	...
	'setting_functions_threadexp_filterednovote' => 'Low-content posts cannot be voted on',
	'setting_functions_threadexp_filterednovote_comment' => 'When enabled, posts identified as low-content cannot receive votes.',
	...
	'setting_editor_switch_enable_comment' => '选择否将禁止用户在纯文本模式和所见即所得模式之间切换',
	...
	'setting_permissions_maxsubjectsize_comment' => '论坛、门户、日志均受限制，管理组成员可通过“发帖不受限制”设置而在论坛模块不受影响, 255 为默认值, 255 为最大值',
...
<br>(3)SPH_RANK_NONE，禁用评分的模式，这是最快的模式。实际上这种模式与布尔搜索相同。所有的匹配项都被赋予权重1',
	...
	'setting_uc_tips' => '<li>本设置在站点安装时自动生成，一般情况下请不要修改，修改前请备份 config/config_global.php 文件，以防止修改错误导致站点无法运行</li>',
	'setting_uc_linkstatus' => '通信情况',
	'setting_uc_linkstatus_correct' => '通信成功',
	'setting_uc_linkstatus_unfixed' => '通信失败，请检查通信配置，使用 UCenter 提供的 "应用的 UCenter 配置信息" 覆盖 config/config_global.php 文件内的常量或尝试设置 UCenter IP 地址',
	...
	'setting_dbhost' => 'UCenter 数据库服务器',
	'setting_dbhost_comment' => '可以是本地也可以是远程数据库服务器，如果 MySQL 端口不是默认的 3306，请填写如下形式：127.0.0.1:6033',
	...
	'styles_home_tips' => '<li>手机版模板需要开启手机版以后才能生效，<a href="'.ADMINSCRIPT.'?action=setting&operation=mobile">前往手机版设置</a></li><li>如果您是模板开发者，请阅读《<a href="https://open.dismall.com/?ac=document&amp;page=dev_template" target="_blank">Discuz! 技术文档</a>》中的内容，并开启设计模式。</li>',
	...
	'usergroups_edit_allowretag_comment' => 'Set whether users in this group can modify tags of other users\' threads (changes will be logged in thread moderation history)',
	...
	'misc_link_tips' => '<li>To hide friend links on the forum index, disable them in "<a href="'.ADMINSCRIPT.'?action=setting&operation=styles&anchor=index">Interface Settings</a>".</li><li>Items without a description are shown in compact form.</li>
						<li>Each link belongs to one group; groups appear as collapsible sections on the forum index.</li>',
	...
	'adv_type_headerbanner_tips' => '<li>展现方式: 页头通栏广告显示于论坛页面右上方，通常使用 468x60 图片 的形式。当前页面有多个页头通栏广告时，系统会随机选取其中之一显示。</li><li>价值分析: 由于能够在页面打开的第一时间将广告内容展现于最醒目的位置，因此成为了网页中价位最高、最适合进行商业宣传或品牌推广的广告类型之一。</li>',
	'adv_type_footerbanner' => '页尾通栏广告',
	'adv_type_footerbanner_tips' => '<li>展现方式: 页尾通栏广告显示于论坛页面中下方，通常使用 468x60 或其他尺寸图片 的形式。当前页面有多个页尾通栏广告时，系统会随机选取其中之一显示。</li><li>价值分析: 与页面头部和中部相比，页面尾部的展现机率相对较低，通常不会引起访问者的反感，同时又基本能够覆盖所有对广告内容感兴趣的受众，因此适合中性而温和的推广。</li>',
	'adv_type_text' => '页内文字广告',
	'adv_type_text_tips' => '<li>展现方式: 页内文字广告以表格的形式，显示于首页、主题列表和帖子内容三个页面的中上方，通常使用文字的形式，也可使用小图片。当前页面有多个文字广告时，系统会以表格的形式按照设定的显示顺序全部展现，同时能够对表格列数在 3～5 的范围内动态排布，以自动实现最佳的广告排列效果。</li><li>价值分析: 由于此类广告通常以文字形式展现，但其所在的较靠上的页面位置，使得此类广告成为了访问者必读的内容之一。同一页面可以呈现多达十几条文字广告的特性，也决定了它是一种平民化但性价比较高的推广方式，同时还可用于论坛自身的宣传和公告之用。</li>',
	...
	'adv_type_interthread_tips' => '<li>展现方式: 帖间通栏广告显示于主题帖和第一个回帖之间，可使用 468x60 或其他尺寸图片 的形式。当前页面有多个帖间通栏广告时，系统会随机选取其中之一显示。</li><li>价值分析: 由于能够将主题与回帖分开，广告尺寸大而且不影响帖子内容，因此不会招致帖子作者及访问者反感，适合在帖内进行商业宣传或品牌推广。</li>',
	'adv_type_float' => '漂浮广告',
	'adv_type_float_tips' => '<li>展现方式: 漂浮广告展现于页面左下角，当页面滚动时广告会自行移动以保持原来的位置，通常使用小图片 的形式。当前页面有多个漂浮广告时，系统会随机选取其中之一显示。</li><li>价值分析: 漂浮广告是进行强力商业推广的有效手段，其在页面中的浮动性，使其与固定的图片和文字相比，更容易被关注，正因为如此，这种强制性的关注也可能招致对此广告内容不感兴趣的访问者的反感。请注意不要将过大的图片 以漂浮广告的形式显示，以免影响页面阅读。</li>',
	'adv_type_couplebanner' => '对联广告',
	'adv_type_couplebanner_tips' => '<li>展现方式: 对联广告以长方形图片的形式显示于页面顶部两侧，形似一幅对联，通常使用宽小高大的长方形图片 的形式。对联广告一般只在使用像素约定主表格宽度的情况下使用，如使用超过 90% 以上的百分比约定主表格宽度时，可能会影响访问者的正常浏览。当访问者浏览器宽度小于 800 像素时，自动不显示此类广告。当前页面有多个对联广告时，系统会随机选取其中之一显示。</li><li>价值分析: 对联广告由于只展现于高分辨率(1024x768 或更高)屏幕的两侧，只占用页面的空白区域，因此不会招致访问者反感，能够良好的突出推广内容。但由于对分辨率和主表格宽度的特殊要求，使得广告的受众比例无法达到 100%。</li>',
	'adv_type_intercat' => '分类间广告',
	'adv_type_intercat_tips' => '<li>展现方式: 分类间广告显示于论坛首页相邻的两个论坛分类之间，可使用 468x60 或其他尺寸图片 的形式。当前页面有多个分类间广告时，系统会从中抽取与论坛分类数相等的条目进行随机显示。</li><li>价值分析:由于出现在论坛首页比较明显的位置，广告展示效果较好，但是过多过大的首页广告可能会招致访问者反感。 </li>',
	'adv_type' => '广告类型',
	'adv_style' => '样式',
	'adv_style_code' => '代码',
	'adv_style_text' => '文字',
	...
	'adv_edit_position_thread_comment' => '帖子内容上方和下方的广告适合使用文字形式，而帖子右侧广告适合使用图片 形式，也可以同时显示多条文字广告',
