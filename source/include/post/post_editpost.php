...
if(!submitcheck('editsubmit')) {
	...
	$modpost->param('redirecturl', "forum.php?mod=viewthread&tid={$_G['tid']}".($_GET['page']?"&page={$_GET['page']}":'')."&extra=$extra#pid$pid");
	...
	if($_G['forum_auditstatuson']) {
		...
		if(!empty($_GET['delete']) && $thread['replies'] == 0) {
			...
			showmessage('article_delete_success', "forum.php?mod=viewthread&tid={$_G['tid']}".($_GET['page']?"&page={$_GET['page']}":'')."&extra=$extra", $param);
		} else {
