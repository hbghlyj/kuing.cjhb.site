...
$result = DB::fetch_all('SELECT a.tid,a.replies,a.lastpost,a.fid IN (2,9,10) as excl FROM ' . DB::table("forum_thread") . ' a ORDER BY a.lastpost DESC '.DB::limit(0, 20000));
...
foreach ($result as $row) {
    ...
    for ($i = 1; $i <= ceil($row['replies'] / 20); $i++) {
        ...
        } elseif ($row['replies'] > 2) {
            $sitemap .= "<priority>0.7</priority>\n";
        } elseif ($row['replies'] > 1) {
