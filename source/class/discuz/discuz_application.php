	...
	private function _init_session() {
		if (empty($_SERVER['HTTP_USER_AGENT']) || strlen($_SERVER['HTTP_USER_AGENT']) < 2) {
			...
			if (isset($_SERVER['HTTP_X_KNOWN_BOT'])) {
				...
				$ip_long = ip2long($_G['clientip']);
				function _check_ip_in_cidrs($cidrs, $user_agent_keyword) {
					global $ip_long;
					foreach ($cidrs as $cidr) {
						...
						if ($subnet_long !== false) {
							...
							if (($ip_long & $mask_long) == ($subnet_long & $mask_long)) {
								DB::query('UPDATE common_robot_user_agents SET last_seen_at = CURRENT_TIMESTAMP(), seen_count = seen_count + 1 WHERE user_agent_keyword = ' . $user_agent_keyword);
								define('IS_ROBOT', $user_agent_keyword . "\t");
								return true;
							}
						}
					}
				...
				if (_check_ip_in_cidrs($alibaba_cloud_singapore_cidrs, "Alibaba Cloud Singapore") == false && _check_ip_in_cidrs($alibaba_cn_net_cidrs, "Alibaba CN Net") == false && _check_ip_in_cidrs($internet_archive_cidrs, "Internet Archive") == false) {
