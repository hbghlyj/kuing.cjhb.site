...
class table_forum_post extends discuz_table {
	...
	public function fetch_all_by_authorid($tableid, $authorid, $outmsg = true, $order = '', $start = 0, $limit = 0, $first = null, $invisible = null, $fid = null, $filterfid = null, $orderfield = '') {
		...
		$orderfield = $orderfield ?: 'dateline';
	...
	public function update_post($tableid, $pid, $data, $unbuffered = false, $low_priority = false, $first = null, $invisible = null, $fid = null, $status = null) {
		...
		if($return && $this->_allowmem) {
			$this->update_cache_post($tableid, $pid, 'pid', $data, ['invisible' => $invisible, 'first' => $first, 'fid' => $fid, 'status' => $status]);
		}
		return $return;
	}
	...
	public function update_by_tid($tableid, $tid, $data, $unbuffered = false, $low_priority = false, $first = null, $invisible = null, $status = null) {
		...
		if($data) {
			$return = DB::update(self::get_tablename($tableid), $data, implode(' AND ', $where), $unbuffered, $low_priority);
			if($return && $this->_allowmem) {
				$this->update_cache_post(0, $tid, 'tid', $data, ['first' => $first, 'invisible' => $invisible, 'status' => $status]);
			}
		}
		return $return;
	}
	...
	public function increase_status_by_pid($tableid, $pid, $status, $glue, $unbuffered = false) {
		...
		if($return && $this->_allowmem) {
			$this->update_cache_post($tableid, $pid, 'pid', ['status' => $status], [], $glue);
		}
		return $return;
	}
	...
	public function fetch_all_by_search($tableid, $tid = null, $keywords = null, $invisible = null, $fid = null, $authorid = null, $author = null, $starttime = null, $endtime = null, $first = null, $start = null, $limit = null) {
		...
		if($sql) {
			return DB::fetch_all('SELECT * FROM %t WHERE 1 %i ORDER BY position DESC %i', [self::get_tablename($tableid), $sql, DB::limit($start, $limit)]);
		} else {
			return [];
		}
	}
