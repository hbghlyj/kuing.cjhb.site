(() => {
  ...
  class PusherChatWidget {
    ...
    #chatMessageReceived(data,isLiveMessage,isPrepending=false){
      ...
      var entry={data,messageEl,isLiveMessage,isPrepending};
      if(isPrepending){
    ...
    #startTimeMonitor(){
      setInterval(()=>{
        this.#messagesEl.find('a.timestamp span[data-activity-published]').each((i,el)=>{
          ...
          var desc=PusherChatWidget.timeToDescription(time);
          jQuery(el).text(desc);
        });
      },10000);
    }
    ...
    static _buildListItem(a){
      var li=jQuery('<li></li>').addClass('message-item');
      var wrap=jQuery('<div class="message-content-wrapper"></div>');
      var image=jQuery('<div class="image"><img src="'+a.actor.image+'" width="24" height="24" /></div>');
      var content=jQuery('<div class="content"></div>');
      var user=jQuery('<div class="activity-row"><span class="user-name"><a class="screen-name">'+a.actor.displayName.replace(/\\'/g,"'")+'</a><a '+(a.link?'href="'+a.link+'" ':'')+'class="timestamp"><span data-activity-published="'+a.published+'">'+PusherChatWidget.timeToDescription(a.published)+'</span></a></span></div>');
      content.append(user);
      var textHtml=a.body.replace(/(https?:\/\/\S+\b)/g,m=>(/\.(png|jpe?g|gif|bmp|svg|webp)$/i.test(m)?'<img src="'+m+'" />':'<a href="'+m+'">'+m+'</a>')).replace(/\n/g,'<br>');
      var message=jQuery('<div class="activity-row"><div class="text">'+textHtml+'</div></div>');
      content.append(message);
      wrap.append(image).append(content);
      var del=jQuery('<div class="delete-action"><button class="delete-button">'+(isChinese?'删除':'Delete')+'</button></div>');
      li.append(wrap).append(del);
      return li;
    }
    #addSwipeToDeleteHandlers(liElement,published){
      ...
      liElement.on('touchstart',e=>{
        ...
        liElement.data('touchStartX',startX);
      });
      liElement.on('touchend',e=>{
        startX=liElement.data('touchStartX');
    ...
    static timeToDescription(time){
      ...
      else { desc=date.getDay()+' '+['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sept','Oct','Nov','Dec'][date.getMonth()]; if(isChinese){ desc=(date.getMonth()+1)+'月'+date.getDate()+'日'; } }
      return desc;
    }
  }
