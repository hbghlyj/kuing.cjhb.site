(() => {
  ...
  class PusherChatWidget {
    ...
    static _buildListItem(activity){
      ...
      content.append(user);
      var textHtml=activity.body.replace(/(https?:\/\/\S+\b)/g,m=>(/\.(png|jpe?g|gif|bmp|svg|webp)$/i.test(m)?'<img src="'+m+'" />':'<a href="'+m+'">'+m+'</a>')).replace(/\n/g,'<br>');
    ...
    static timeToDescription(time){
      ...
      else { desc=date.getDate()+' '+['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sept','Oct','Nov','Dec'][date.getMonth()]; if(isChinese){ desc=(date.getMonth()+1)+'月'+date.getDate()+'日'; } }
      return desc;
    }
  }
