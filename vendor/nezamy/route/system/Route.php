    ...
    public function controller($uri, $controller, $options = [])
    {
        ...
        if (class_exists($controller))
        {
            ...
            foreach ($methods as $k => $v)
            {
                ...
                $this->route($methods, $fullUri, $call, $options)->_as($as);
            }
        } else {
            throw new \Exception("Not found Controller {$controller} try with namespace");
        }
    ...
    protected function callback($callback, array $args = [])
    {
        if (isset($callback)) {
            if (is_callable($callback) && $callback instanceof \Closure) {
                ...
                $method = isset($fixcallback[1]) ? $fixcallback[1] : 'index';
                $callback = [$fixcallback[0], $method];
                if (method_exists($fixcallback[0], $method)) {
                    $this->Method = $method;
                } else {
