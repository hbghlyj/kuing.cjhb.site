...
var getBasePath = function() {
	...
	len = els.length; i < len; i++) {
		src = els[i].src || '';
		if (/discuz_uploader[\w\-\.]*\.js/.test(src)) {
...
if (DiscuzUploader == undefined) {
	DiscuzUploader = function(settings) {
		this.initUploader(settings);
	};
}

DiscuzUploader.CURSOR = {
	ARROW: -1,
	HAND: -2
};

DiscuzUploader.EXT_MIME_MAP = {
	'3gp': 'video/3gpp',
...
DiscuzUploader.prototype.initUploader = function(userSettings) {
	try {
		this.customSettings = {};
		this.settings = {};
		this.eventQueue = [];
...
DiscuzUploader.prototype.initSettings = function (userSettings) {
	...
	uploader.on('fileQueued', function(file) {
		...
		if(self.customSettings.uploadType == 'image' && /^image\//i.test(file.type || '') && typeof URL != 'undefined' && typeof URL.createObjectURL == 'function') {
			file.previewObjectUrl = URL.createObjectURL(file.source.source);
			setUploadPreview(file, self.customSettings.progressTarget, file.previewObjectUrl);
		}
...
DiscuzUploader.prototype.setUploadURL = function (url) {
	this.uploader.options.server = url.toString();
};

DiscuzUploader.prototype.addPostParam = function (name, value) {
...
DiscuzUploader.prototype.executeNextEvent = function () {
...
DiscuzUploader.prototype.debug = function (message) {
	this.queueEvent("debug_handler", message);
};

DiscuzUploader.prototype.debugMessage = function (message) {
	...
	if (this.settings.debug) {
		...
		if (typeof message === "object" && typeof message.name === "string" && typeof message.message === "string") {
			...
			DiscuzUploader.Console.writeLine(exceptionMessage);
		} else {
			DiscuzUploader.Console.writeLine(message);
		}
	}
};
...
FileProgress.prototype.toggleCancel = function(show, uploaderInstance) {
	this.fileProgressElement.childNodes[0].style.display = show ? "": "none";
	if (uploaderInstance) {
		var fileID = this.fileProgressID;
		var oSelf = this;
		this.fileProgressElement.childNodes[0].onclick = function() {
			uploaderInstance.cancelFile(fileID);
...
FileProgress.prototype.disappear = function() {
	...
	if (this.height > 0 || this.opacity > 0) {
		...
		releaseUploadPreview(this.file);
		this.setTimer(null);
	}
};
